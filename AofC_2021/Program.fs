...
let generateReadMe (types: Type array) (basePath: DirectoryInfo) =
...
let writeReadme readmeFilename =
    ...
    if readmeFile.IsSome then
        ...
        if startMatch.Success then
            let autogenerated = generateReadMe (getDayTypes |> Map.toArray |> Array.map (fun f -> snd f)) (findFileInParents2 "Program.fs").Value.Directory
